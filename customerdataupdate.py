...
def showcusupdate():
    ...
    def updatedata():
        ...
        xd = e4.get()
        xe = e5.get()
        sql = "update customerdata set name='%s', address='%s', email='%s', phone='%s' where custid=%d" % (xb, xc, xd, xe, xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    btn3=Button(t,text='Update',command=updatedata,bg='#1565c0',fg='white',font=('calibri',11,'bold'),width=10,height=1)
    btn3.place(x=300,y=380)
    btn4=Button(t, text='Find',command=finddata,bg='#1565c0',fg='white',font=('calibri',11,'bold'),width=10,height=1)
    btn4.place(x=400, y=380)
